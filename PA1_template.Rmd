...
The mean total number of steps taken per day is `r round(meansteps,2)`
...
`r format(times[which.max(intervalsteps)],'%H:%M')`
...
```
The total number of missing values (rows with NAs) is `r na.count`
...
The median total number of steps taken per day is `r round(medsteps,2)`
